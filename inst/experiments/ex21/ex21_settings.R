...
tcor_ocor_margs <- list(
  ...
  pcov_model_0 = list(
    method  = glm,
    formula = Y ~ fA + Z1_abs + Z2 + Z1_abs*Z2 + ipw0,
    options = list(
      family  = gaussian(link = 'identity'))
  ),
  pcov_model_1 = list(
    method  = glm,
    formula = Y ~ fA + Z1_abs + Z2 + Z1_abs*Z2 + ipw1,
    options = list(
      family  = gaussian(link = 'identity'))
  )
)
...
tcor_omis_margs <- list(
  ...
  pcov_model_0 = list(
    method  = glm,
    formula = Y ~ fA + Z1_abs + Z2 + ipw0,
    options = list(
      family  = gaussian(link = 'identity'))
  ),
  pcov_model_1 = list(
    method  = glm,
    formula = Y ~ fA + Z1_abs + Z2 + ipw1,
    options = list(
      family  = gaussian(link = 'identity'))
  )
)
