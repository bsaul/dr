...
sims <- split(sims_100x_m300_n4_s198_ex06, sims_100x_m300_n4_s198_ex06$simID)
library(doMC)
registerDoMC(4)
...
infer_estimates <- plyr::llply(sims, .parallel = TRUE, function(x){
  interference(Y | A ~ Z1 + (1|group) | group,
                           data = x,
                           allocations = allocations)$estimates %>%
...
infer_results <- infer_estimates %>%
  bind_rows() %>%
  group_by(alpha1, trt1) %>%
  summarise(meanest = mean(estimate))

save(infer_estimates, estimates, oracle, file = paste0('development/experiments/ex06/simresults_', Sys.Date(), '.rda'))
