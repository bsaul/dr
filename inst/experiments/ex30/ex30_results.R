...
load(file = 'inst/experiments/ex30/ex30_results.rda')
...
plot_two <- function(.sid, .model_spec, .method_labels, .alpha, .a){
  arrangeGrob(
    grobs = lapply(c('ipw', 'otc', 'dbr', 'wls_dbrwls'), function(x) {
      plot_one(.sid         = .sid, 
...
coverage_plot_axes <- ggplot(
  ...
  theme_void() + 
  theme(
    plot.margin  = unit(c(0, 0, 0, 0.85), 'cm'),
    axis.text.y  = element_text(size = 7, hjust = 1),
    axis.ticks.y = element_blank(),
    axis.title.y = element_text(size = 7, hjust = 1),
    axis.text.x  = element_blank(),
...
ggsave(filename = paste0('inst/experiments/ex30/figures/', 'y_1_5', '.pdf'),
       plot = y_1_5,
       width = 8.5, height = 5)
...
ggsave(filename = paste0('inst/experiments/ex30/figures/', 'y_0_5', '.pdf'),
       plot = y_0_5,
       width = 8.5, height = 5)
...
lapply(c(9), function(sid){
  ...
  ggsave(filename = paste0('inst/experiments/ex30/figures/scn', sid, '.pdf'),
         plot = p,
         width = 8.5, height = 4)
})
