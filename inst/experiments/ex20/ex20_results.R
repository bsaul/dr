...
plot_two <- function(.sid, .model_spec, .method_labels, .alpha, .a){
  arrangeGrob(
    grobs = lapply(c('ipw', 'otc', 'dbr'), function(x) {
      plot_one(.sid         = .sid, 
...
ggsave(filename = paste0('inst/experiments/ex20/figures/', 'y_1_5', '.pdf'),
       plot = y_1_5,
       width = 8.5, height = 5)
...
ggsave(filename = paste0('inst/experiments/ex20/figures/', 'y_0_5', '.pdf'),
       plot = y_0_5,
       width = 8.5, height = 5)
