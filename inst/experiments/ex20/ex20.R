#------------------------------------------------------------------------------#
#   Title: Development experiment 20
#  Author: B. Saul
#    Date: 2017-02-25
...
library(doMC)
registerDoMC(4)
source('inst/experiments/ex20/ex20_funs.R')
source('inst/experiments/ex20/ex20_settings.R')
nsims <- 500
which_scenarios <- 9:12

## Compute estimates ##
allocations <- c(0.1, 0.5, 0.9)
# ptm <- proc.time()
estimates <- do_scenarios(nsims, which_scenarios, allocations, all_model_args = margs) %>%
   bind_rows()
...
save(estimates, scenarios, oracle, results, nsims, margs,
     file = 'inst/experiments/ex20/ex20_results.rda')
