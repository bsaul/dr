...
```{r data}
load(file = 'ex13_results.rda')
```

The bias from IPW (i.e. `inferference`), outcome, and doubly robust estimators of Liu et al was assessed for the following data generating scenarios. $n_i = 20$ for $m = 300$ groups. The simulation was run on 300 datasets.
...
estimates %>%
  ...
  facet_grid(a ~  method + model_spec)
```
