...
make_bias_plot <- function(.sid, .model_spec){
  plot_dt <- estimates %>% filter(sid == .sid, model_spec == .model_spec, !is.na(a)) %>%
...
arrange_bias_plots <- function(.sid, row_height){
  ...
  # Make the bias plots for all model specifications
    bias_plots <- lapply(seq_along(margs), function(i) {
      spec <- names(margs[i])
      p <- make_bias_plot(.sid = .sid, .model_spec = spec) 
        ...
        arrangeGrob(
          grobs = list(
            ...
            ggplotGrob(p)),
          nrow = 2,
          heights = unit(c(.2, row_height), 'in')
        )
      })
  ...
  arrangeGrob(
    grobs   = bias_plots,
    nrow    = nrows,
    heights = unit(rep(.2 + row_height, nrows), 'in')
  )
}
