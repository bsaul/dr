...
sims <- split(sims_250x_m300_n10_s198_X010, sims_250x_m300_n10_s198_X010$simID)
...
estimates <- plyr::llply(sims, .parallel = TRUE, function(x){
  ...
  ## Component models
  tmodel <- lme4::glmer(A ~ Z1 + Z2 + Z3 + Z4 + (1|group) , data = x, 
                        family = binomial(link = 'logit') )
  omodel <- geepack::geeglm(Y ~ Z1 + Z2 + Z3 + Z4 + A + fA + A*Z1 + fA*Z2, data = x, 
...
target_a <- 0
oracle <- data.frame(alpha = allocations, a = target_a, truth = 2+0.5*target_a+6*(allocations*3/4))
